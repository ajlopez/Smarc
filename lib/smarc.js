...
function ContractDefinition(code) {
	...
	definition.methods.private = definition.methods.private || {};
	definition.properties = definition.properties || {};
	definition.properties.private = definition.properties.private || [];
	...
	this.instance = function (args, storage) {
		...
		for (n in definition.methods.private)
			contract[n] = definition.methods.private[n];
	
		for (n in definition.properties.public)
			defineProperty(storage, contract, definition.properties.public[n]);
		for (n in definition.properties.private)
			defineProperty(storage, contract, definition.properties.private[n]);
